...
import { SalidaProduccionService } from 'src/repositories/movimientos-recurso/salida/salida-produccion.service';
import { NotaVentaEntity } from 'src/entities/DocumentosFuente/DocumentosTransaccion/NotaVenta/NotaVentaEntity';
import { DbPresetService } from 'src/repositories/preset/db-preset.service';
import { v4 } from 'uuid';
...
export class NotaVentaService {
    ...
    async executeCreateCollection( s: SessionData, notasVenta: NotaVenta[] )
    {
        ...
        await NotaVentaEntity.bulkCreate( notasVenta.map( notaVenta => ({
            id: notaVenta.id,
        ...
        await this.entradaEfectivoService.executeCreateCollection( s, notasVenta.flatMap( notaVenta => notaVenta.entradasEfectivo ) );
        await this.salidaBienConsumoService.executeCreateCollection( s, notasVenta.flatMap( notaVenta => notaVenta.salidasBienConsumo ) );
        await this.salidaProduccionService.executeCreateCollection( s, notasVenta.flatMap( notaVenta => notaVenta.salidasProduccionServicioReparacion ) );
    }


    async executeDeleteCollection( s: SessionData, notasVenta: NotaVenta[] )
    {
        return await this.documentoTransaccionService.executeDeleteCollection( s, notasVenta );
    }


    async setCode( s: SessionData, notasVenta: NotaVenta[] )
    {
        return await this.documentoTransaccionService.setCode( s, notasVenta );
    }
    ...
    {
        return await this.documentoTransaccionService.updateVoid( s, notaVenta );
    }
