...
import { SalidaBienConsumoRepository } from 'src/infrastructure/repositories/movimientos-recurso/salida/salida-bien-consumo.service';
import { SalidaProduccionRepository } from 'src/infrastructure/repositories/movimientos-recurso/salida/salida-produccion.service';
import { DbPresetRepository } from 'src/infrastructure/repositories/preset/db-preset.service';
...
export class NotaVentaRepository {
    ...
    constructor(
        ...
        private salidaBienConsumoRepository: SalidaBienConsumoRepository,
        private salidaProduccionRepository: SalidaProduccionRepository,
        private dbPresetRepository: DbPresetRepository
    )
    {}
    ...
    {
        await this.documentoTransaccionRepository.executeCreateCollection( s, notasVenta );

        await NotaVentaOrm.bulkCreate( notasVenta.map( notaVenta => ({
            id: notaVenta.id,
            fechaCompromiso: notaVenta.fechaCompromiso,
            clienteUuid: notaVenta.cliente?.uuid,
            nvPrioridadId: notaVenta.prioridad?.id,
            usuarioTecnicoUuid: notaVenta.usuarioTecnico?.uuid,
            nvEstadoId: notaVenta.estado?.id
        }) ), {
            transaction: s.transaction
        } );

        await this.entradaEfectivoRepository.executeCreateCollection( s, notasVenta.flatMap( notaVenta => notaVenta.entradasEfectivo ) );
        await this.salidaBienConsumoRepository.executeCreateCollection( s, notasVenta.flatMap( notaVenta => notaVenta.salidasBienConsumo ) );
        await this.salidaProduccionRepository.executeCreateCollection( s, notasVenta.flatMap( notaVenta => notaVenta.salidasProduccionServicioReparacion ) );
    }


    async executeDeleteCollection( s: SessionData, notasVenta: NotaVenta[] )
    {
        return await this.documentoTransaccionRepository.executeDeleteCollection( s, notasVenta );
    }


    async setCode( s: SessionData, notasVenta: NotaVenta[] )
    {
        return await this.documentoTransaccionRepository.setCode( s, notasVenta );
    }
    ...
    async getObjectById( s: SessionData, notaVenta: NotaVenta )
    {
        const data = await this.conectorService.executeQuery({
            ...
            query: `
                ${this.query}
                and nota_venta.id ${notaVenta.id === undefined ? ' is null ': ' = :id '}
            `,
            parameters: {
    ...
    async create( s: SessionData, notaVenta: NotaVenta )
    {
        const almacenProductos = await this.dbPresetRepository.getObjectByTarget( s, new DbPreset({ target: 'nv_salida_bien_consumo.almacen_uuid' }) );
        if ( almacenProductos.valor === undefined ) throw new InternalServerErrorException( `${almacenProductos.titulo} NO ESTABLECIDO` );

        const almacenProductosServicioReparacion = await this.dbPresetRepository.getObjectByTarget( s, new DbPreset({ target: 'nv_servicio_reparacion_recurso_bien_consumo.almacen_uuid' }) );
        ...
        notaVenta.set({
            ...
            salidasBienConsumo: notaVenta.salidasBienConsumo.map( sal => sal.set({
                almacen: new Almacen({
                    uuid: almacenProductos.valor
                })
            }) ),
            salidasProduccionServicioReparacion: notaVenta.salidasProduccionServicioReparacion.map( sal => sal.set({
        ...
        .setRelation({
            ...
            salidaProduccionId: await this.conectorService.getId( s.transaction, 'salida_produccion' ),
            notaVentaSalidaProduccionServicioReparacionRecursoBienConsumoId: await this.conectorService.getId( s.transaction, 'nv_servicio_reparacion_recurso_bien_consumo' ),
            notaVentaSalidaProduccionServicioReparacionRecursoServicioId: await this.conectorService.getId( s.transaction, 'nv_servicio_reparacion_recurso_servicio' )
        })
        .procesarInformacion();


        await this.executeCreateCollection( s, [ notaVenta ] );
        return await this.getObjectById( s, notaVenta );
    }


    async createAndIssue( s: SessionData, notaVenta: NotaVenta )
    {
        ...
        notaVenta.set({
            ...
            entradasEfectivo: notaVenta.entradasEfectivo.map( ent => ent.set({
                uuid: v4()
            }) ),
            docsEntradaEfectivo: notaVenta.docsEntradaEfectivo.map( doc => doc.set({
                uuid: v4(),
                entradas: doc.entradas.map( ent => ent.set({
                    uuid: v4()
                }) )
            }) ),
            docsEntradaBienConsumo: notaVenta.docsEntradaBienConsumo.map( doc => doc.set({
                uuid: v4(),
                entradas: doc.entradas.map( ent => ent.set({
                    uuid: v4()
                }) )
            }) ),
            docsSalidaEfectivo: notaVenta.docsSalidaEfectivo.map( doc => doc.set({
                uuid: v4(),
                salidas: doc.salidas.map( sal => sal.set({
                    uuid: v4()
                }) )
            }) ),
            docsSalidaBienConsumo: notaVenta.docsSalidaBienConsumo.map( doc => doc.set({
                uuid: v4(),
                salidas: doc.salidas.map( sal => sal.set({
                    uuid: v4()
                }) )
        ...
        await this.setCode( s, [ notaVenta ] );


        const docsMovimiento = [
            ...notaVenta.docsEntradaEfectivo,
            ...notaVenta.docsEntradaBienConsumo,
            ...notaVenta.docsSalidaEfectivo,
            ...notaVenta.docsSalidaBienConsumo
        ];
    ...
    async update( s: SessionData, notaVenta: NotaVenta )
    {
        const item2validate = await this.getObjectById( s, notaVenta );
    ...
    async updateAndIssue( s: SessionData, notaVenta: NotaVenta )
    {
        const dateTimeEmision = Prop.toDateTime( notaVenta.fechaEmision );
        if ( !dateTimeEmision.isValid ) throw new InternalServerErrorException( ERROR_DOCUMENT.DATETIME_ISSUE_INVALIDATE );
    ...
    async updateVoid( s: SessionData, notaVenta: NotaVenta )
    {
        return await this.documentoTransaccionRepository.updateVoid( s, notaVenta );
    }


    async delete( s: SessionData, notaVenta: NotaVenta )
    {
        const af1 = await this.executeDeleteCollection( s, [ notaVenta ] );
