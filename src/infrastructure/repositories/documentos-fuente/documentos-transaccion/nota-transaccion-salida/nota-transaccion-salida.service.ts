...
import { SalidaEfectivoRepository } from 'src/infrastructure/repositories/movimientos-recurso/salida/salida-efectivo.service';
...
@Injectable()
export class NotaTransaccionSalidaRepository {
    ...
    constructor(
        ...
        private salidaEfectivoRepository: SalidaEfectivoRepository
    )
    {}
    ...
    {
        await this.documentoTransaccionRepository.executeCreateCollection( s, ntss );

        await NotaTransaccionSalidaOrm.bulkCreate( ntss.map( nts => ({
            id: nts.id,
            ...
            ntsDetallesOrm: nts.detalles.map( detalle => new NtsDetalleOrm({
                id: detalle.id,
        ...
        await this.salidaEfectivoRepository.executeCreateCollection( s, ntss.filter( nts => nts.liquidacion?.id === 2 )
            .map( nts => nts.credito )
    ...
    {
        return await this.documentoTransaccionRepository.executeDeleteCollection( s, ntss );
    }


    async setCode( s: SessionData, ntss: NotaTransaccionSalida[] )
    {
        return await this.documentoTransaccionRepository.setCode( s, ntss );
    }
    ...
    {   
        ...
        nts.set({
            ...
            docsEntradaEfectivo: nts.docsEntradaEfectivo.map( doc => doc.set({
                uuid: v4(),
                entradas: doc.entradas.map( ent => ent.set({
                    uuid: v4()
                }) )
            }) ),
            docsEntradaBienConsumo: nts.docsEntradaBienConsumo.map( doc => doc.set({
                uuid: v4(),
                entradas: doc.entradas.map( ent => ent.set({
                    uuid: v4()
                }) )
            }) ),
            docsSalidaEfectivo: nts.docsSalidaEfectivo.map( doc => doc.set({
                uuid: v4(),
                salidas: doc.salidas.map( sal => sal.set({
                    uuid: v4()
                }) )
            }) ),
            docsSalidaBienConsumo: nts.docsSalidaBienConsumo.map( doc => doc.set({
                uuid: v4(),
                salidas: doc.salidas.map( sal => sal.set({
                    uuid: v4()
                }) )
    ...
    {
        return await this.documentoTransaccionRepository.updateVoid( s, nts );
    }
