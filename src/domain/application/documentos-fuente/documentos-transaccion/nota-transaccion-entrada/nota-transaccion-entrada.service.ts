...
export class NotaTransaccionEntradaService {
    ...
    async executeCreateCollection( s: SessionData, items: NotaTransaccionEntrada[], withSupertype: boolean = true )
    {
        if ( withSupertype )
            await this.documentoTransaccionService.executeCreateCollection( s, items );

        await NotaTransaccionEntradaOrm.bulkCreate( items.map( item => ({
            id: item.id,
            comprobanteTipoId: item.comprobanteTipo?.id,
            comprobanteCodigoSerie: item.comprobanteCodigoSerie,
            comprobanteCodigoNumero: item.comprobanteCodigoNumero,
            proveedorUuid: item.proveedor?.uuid,
            proveedorDocumentoIdentificacionUuid: item.proveedorDocumentoIdentificacion?.uuid,
            proveedorCodigo: item.proveedorCodigo,
            proveedorNombre: item.proveedorNombre,
            proveedorCelular: item.proveedorCelular,
            liquidacionTipoId: item.liquidacion?.id,
            nteDetallesOrm: item.detalles.map( detalle => new NteDetalleOrm({
        ...
        await this.entradaEfectivoService.executeCreateCollection( s, items.filter( item => item.liquidacion?.id === 2 )
    ...
    async getObjectByUuid( s: SessionData, item: NotaTransaccionEntrada )
    {
    ...
    async create( s: SessionData, item: NotaTransaccionEntrada )
    {
        item.set({
            uuid: v4(),
    ...
    async createAndIssue( s: SessionData, item: NotaTransaccionEntrada )
    {   
        ...
        item.set({
            uuid: v4(),
            fechaAnulacion: undefined,
            codigoSerie: `NTE${item.dateTimeEmision.toFormat( 'yyyy' )}`,
            usuario: s.usuarioSession,
            detalles: item.detalles.map( detalle => detalle.set({
                uuid: v4()
            }) ),
    ...
    async update( s: SessionData, item: NotaTransaccionEntrada )
    {
    ...
    async updateVoid( s: SessionData, item: NotaTransaccionEntrada )
    {
        const item2validate = await this.getObjectByUuid( s, item );
        if ( item2validate.fechaAnulacion ) throw new InternalServerErrorException( ERROR_DOCUMENT.CANT_UPDATE_FROM_VOID );
    ...
    async delete( s: SessionData, item: NotaTransaccionEntrada )
    {
        const item2validate = await this.getObjectByUuid( s, item );
