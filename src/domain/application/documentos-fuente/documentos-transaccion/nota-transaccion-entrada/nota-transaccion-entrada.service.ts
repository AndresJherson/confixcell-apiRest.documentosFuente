...
export class NotaTransaccionEntradaService {
    ...
    async executeCreateCollection( s: SessionData, ntes: NotaTransaccionEntrada[] )
    {
        ...
        await NotaTransaccionEntradaOrm.bulkCreate( ntes.map( item => ({
            id: item.id,
        ...
        await this.entradaEfectivoService.executeCreateCollection( s, ntes.filter( item => item.liquidacion?.id === 2 )
    ...
    async updateAndIssue( s: SessionData, item: NotaTransaccionEntrada )
    {
        const dateTimeEmision = Prop.toDateTime( item.fechaEmision );
        if ( !dateTimeEmision.isValid ) throw new InternalServerErrorException( ERROR_DOCUMENT.DATETIME_ISSUE_INVALIDATE );
